...
int main(int argc, char *argv[]) {
	...
	if (!isatty(fileno(stdin))){
		ret_code+=readPipe(&args);
	}
	// ret_code+=parseArgs(argc, argv , &args);

	// repeat(args);
	// returnCode(ret_code,argv[0]);
	return return_code;
}//main
...
UI32 readPipe(ARGS *args){
  ...
  // sPipe = getline(&pipe, &len, stdin);
	...
	// strcpy(args->rep,pipe);
	// pPipe=realloc(args->rep,sRead);
	// for (UI32 i=0;i<sRead;i++){
		// args->rep[i]=pipe[i];
	// }
	// pipe[sRead++]=Z0;

	args->rep=escape(pPipe);

	// args->rep=pPipe;
	// free(pipe);
	printf("rep:\t%s\n",args->rep);
	if (sRead > 1) ret_code+=(1<<0);
